...
const candidateItems=(data.candidates||[]).map((x,i)=>`<section class="candidate"><div class="rank">${String(i+1).padStart(2,"0")}</div><div><span class="tag">${x.relevance_score.toFixed(1)}</span>${x.relevance_verdict?`<span class="tag">${x.relevance_verdict}</span>`:""}${x.relevance_confidence!=null?`<span class="tag">conf ${x.relevance_confidence.toFixed(2)}</span>`:""}<span class="tag">${x.citation_count} cites</span><h3><a href="${x.url||"#"}">${x.title}</a></h3><p>${x.year||""}</p><p class="meta">${x.id} · ${x.discovered_via?.[0]||"n/a"}</p><p>${x.relevance_reason||""}</p></div><aside><div class="eyebrow">Review</div><div class="score">${x.relevance_score.toFixed(1)}</div><ul>${(x.topics||[]).slice(0,3).map(t=>`<li>${t}</li>`).join("")}</ul></aside></section>`).join("");
